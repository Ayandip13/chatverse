...
function processFile(filePath) {
  ...
  content = content.replace(/router\.push\((.*?)\)/g, (match, route) => `navigation.navigate(${mapRoute(route)})`);
  content = content.replace(/router\.replace\((.*?)\)/g, (match, route) => `navigation.replace(${mapRoute(route)})`);
...
['d:/exponew/chatting-platform/apps/boy-app/src', 'd:/exponew/chatting-platform/apps/girl-app/src'].forEach(dir => {
