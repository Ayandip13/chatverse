...
export const useChatSocket = (chatId?: string) => {
  ...
  const [lowBalanceWarning, setLowBalanceWarning] = useState<string | null>(
    null,
  );
  const [endedSummary, setEndedSummary] = useState<ChatEndedSummary | null>(
    null,
  );
  const [disconnectState, setDisconnectState] =
    useState<DisconnectState | null>(null);
  ...
  useEffect(() => {
    ...
    const onTypingStart = ({ chatId: typedChatId }: any) =>
      setTyping(typedChatId, true);
    const onTypingStop = ({ chatId: typedChatId }: any) =>
      setTyping(typedChatId, false);
    ...
    const onStatsUpdate = (data: ChatStatsData) => {
      if (!chatId || data.chatId === chatId) {
        ...
        queryClient.setQueryData(["walletSummary"], (old: any) =>
          old
            ? { ...old, currentBalance: data.remainingCoins }
            : { currentBalance: data.remainingCoins },
        );
      }
    };
    ...
    const onParticipantDisconnected = (data: {
      chatId: string;
      userId: string;
      graceSeconds?: number;
    }) => {
      if (!chatId || data.chatId === chatId) {
        const initialSecs =
          data.graceSeconds !== undefined ? data.graceSeconds : 30;
        setDisconnectState({
          userId: data.userId,
          chatId: data.chatId,
          graceSeconds: initialSecs,
        });
    ...
    const onParticipantReconnected = (data: {
      chatId: string;
      userId: string;
    }) => {
      if (!chatId || data.chatId === chatId) {
    ...
    const onWalletUpdate = ({ newBalance, balance }: any) => {
      ...
      queryClient.setQueryData(["walletSummary"], (old: any) =>
        old
          ? { ...old, currentBalance: updatedBalance }
  ...
  const emitTyping = (chatId: string, isTyping: boolean) => {
    if (socket && isConnected) {
      socket.emit(isTyping ? "chat:typing_start" : "chat:typing_stop", {
        chatId,
      });
    }
  };
