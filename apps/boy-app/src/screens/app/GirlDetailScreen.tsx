...
export default function GirlDetailsScreen() {
  ...
  const { mutateAsync: sendChatRequest, isPending: isSendingRequest } =
    useSendChatRequest();
  const { mutateAsync: cancelChatRequest, isPending: isCancelling } =
    useCancelChatRequest();
  ...
  const activeChat = allChats?.find(
    (c: any) =>
      (c.otherParticipant?._id === id ||
        c.girlId === id ||
        (typeof c.girlId === "object" && c.girlId?._id === id)) &&
      c.status === "ACTIVE",
  );
  ...
  const existingPendingReq = pendingRequests?.find(
    (r: any) =>
      (r.receiverId === id ||
        (typeof r.receiverId === "object" && r.receiverId?._id === id)) &&
      r.status === "PENDING",
  );
  ...
  useEffect(() => {
    ...
    const onRejected = (data: { requestId: string }) => {
      ...
      setActiveRequestId(null);
      ToastAndroid.show(
        "The creator is currently unavailable or declined your request.",
        ToastAndroid.LONG,
      );
    };
    ...
    return (
      <View className="flex-1 bg-white dark:bg-gray-900">
        <View className="w-full h-[450px]">
          ...
          <SafeAreaView
            edges={["top"]}
            className="absolute top-0 w-full flex-row justify-between px-6 pt-4"
          >
      ...
      >
          ...
          <View className="absolute bottom-6 px-6 w-full flex-row justify-between items-end">
            <View>
              <View className="flex-row items-center mb-1">
                <Text className="text-white text-3xl font-extrabold mr-2 shadow-sm">
                  {girl.name}
                </Text>
                {girl.isOnline && (
              ...
              <View className="flex-row items-center bg-black/40 px-3 py-1.5 rounded-full self-start backdrop-blur-md">
                ...
                <Text className="text-white text-xs font-bold">
                  ...
                  <Text className="text-gray-300 font-normal">
                    {" "}
                    ({girl.totalReviews || 0} reviews)
                  </Text>
                </Text>
              </View>
            </View>
        ...
        <View className="px-6 py-6 pb-32">
          ...
          <Text className="text-base text-gray-600 dark:text-gray-300 leading-relaxed mb-6">
            {girl.bio ||
              "Hi there! I'm new here and looking forward to chatting."}
          </Text>
          ...
          {pendingReqId && !activeChat && (
            <View className="bg-indigo-50 dark:bg-indigo-900/30 p-4 rounded-2xl border border-indigo-100 dark:border-indigo-800 flex-row items-center justify-between mb-4">
              ...
              >
                <Text className="text-xs font-bold text-indigo-800 dark:text-indigo-200">
                  Cancel
                </Text>
              </TouchableOpacity>
            </View>
