...
import {
  GirlAvatarCard,
  GirlDetailCard,
} from "../../components/home/GirlCards";
...
export default function HomeScreen() {
  ...
  const { data: recommendedGirls, isLoading: isLoadingRec } =
    useRecommendedGirls();
  const { data: popularGirls, isLoading: isLoadingPop } = usePopularGirls();
  const { data: recentGirls, isLoading: isLoadingRecent } =
    useRecentlyJoinedGirls();
      ...
      return (
        ...
        >
          {[1, 2, 3].map((i) => (
            <Skeleton
              key={i}
              className={`mr-4 ${useGrid ? "w-40 h-64 rounded-2xl" : "w-16 h-16 rounded-full"}`}
            />
          ))}
        </ScrollView>
      );
