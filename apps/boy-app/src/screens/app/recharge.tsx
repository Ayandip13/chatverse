...
export default function RechargeScreen() {
  ...
  const validateAmount = (val: number) => {
    if (val < MIN_RECHARGE) {
      return `Minimum recharge is ₹${MIN_RECHARGE}`;
    }

    if (val > MAX_RECHARGE) {
      return `Maximum recharge is ₹${MAX_RECHARGE}`;
    }
            ...
            onPress: async () => {
              try {
                ...
                await apiClient.post('/wallet/verify', {
                  ...
                  razorpayPaymentId: `pay_mock_${Math.random()
                    .toString(36)
                    .substring(7)}`,
      ...
      >
        ...
        <View className="px-6 py-4 flex-row items-center justify-between border-b border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md">
          ...
          <View className="flex-row items-center bg-indigo-50 dark:bg-indigo-950/60 px-3 py-1.5 rounded-full border border-indigo-100 dark:border-indigo-900/40">
            ...
            <Text className="text-[11px] font-extrabold text-indigo-600 dark:text-indigo-400 uppercase tracking-wide">
              Instant Credit
            </Text>
          </View>
        </View>
        ...
        >
          ...
          <View className="bg-indigo-600 dark:bg-indigo-800 rounded-2xl p-4 relative overflow-hidden mb-5 border border-indigo-500/40 flex-row items-center justify-between">
            {/* Ambient Glow */}
            ...
            <View className="bg-indigo-900/40 dark:bg-indigo-950/50 px-2.5 py-1 rounded-xl border border-indigo-400/20">
              <Text className="text-indigo-200 text-[10px] font-bold">
                1 Coin = 1 Min Talk
              </Text>
            </View>
          </View>
            ...
            {PRESET_PACKAGES.map((pkg) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`w-[48%] p-4 mb-3.5 rounded-3xl border relative overflow-hidden ${isSelected
                ...
                >
                  ...
                  {pkg.isPopular && (
                    <View
                      className={`absolute top-0 right-0 px-2.5 py-0.5 rounded-bl-xl ${isSelected
                          ? 'bg-amber-400'
                          : 'bg-indigo-100 dark:bg-indigo-900/60'
                        }`}
                    >
                      <Text
                        className={`text-[9px] font-black ${isSelected
                            ? 'text-gray-900'
                  ...
                  {pkg.bonus && (
                    <View
                      className={`mt-2 px-2 py-0.5 rounded-lg self-start ${isSelected
                          ? 'bg-white/20'
                          : 'bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/60 dark:border-emerald-900/40'
                        }`}
                    >
                      <Text
                        className={`text-[10px] font-black ${isSelected
                            ? 'text-white'
            ...
            <Input
              ...
              leftIcon={
                <View className="mr-2 items-center justify-center">
                  <Text className="text-gray-500 font-black text-xl">
                    ₹
                  </Text>
                </View>
              }
          ...
          {/* Trust Banner */}
          <View className="bg-white dark:bg-gray-800 rounded-3xl p-4 mb-8 flex-row items-center border border-gray-100 dark:border-gray-700/80">
            ...
            <View className="flex-1">
              <Text className="text-xs font-extrabold text-gray-900 dark:text-white">
                100% Encrypted & Instant Payment
              </Text>

              <Text className="text-[11px] font-semibold text-gray-400 dark:text-gray-400 mt-0.5">
          ...
          <Button
            ...
            className="w-full h-15 rounded-3xl bg-indigo-600 active:bg-indigo-700 border border-indigo-500"
            leftIcon={
              <View className="mr-2 items-center justify-center">
                <CreditCard
                  color="white"
