  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 bg-gray-900 justify-center items-center">
        ...
        <Text className="text-gray-400 mt-4 text-sm font-medium">Loading Developer Config...</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView className="flex-1 bg-gray-900" edges={['top', 'bottom']}>
      ...
      <View className="px-6 py-4 border-b border-gray-800 flex-row items-center justify-between">
        <TouchableOpacity
          ...
          className="w-10 h-10 rounded-full bg-gray-800 items-center justify-center border border-gray-700"
        >
          <ArrowLeft color="#ffffff" size={20} />
        </TouchableOpacity>
        <View className="items-center">
          <Text className="text-lg font-bold text-white tracking-tight">Developer Settings</Text>
          <Text className="text-[10px] font-mono text-indigo-400 uppercase tracking-widest">
            Runtime Switcher
          </Text>
        </View>
        <View className="w-10" />
      ...
      <ScrollView className="flex-1 px-5 pt-6" contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        <View className="bg-gray-800 rounded-3xl p-5 border border-gray-700 mb-6 shadow-lg">
          ...
          <View className="bg-gray-900/80 p-3.5 rounded-2xl border border-gray-800 space-y-2 mb-2">
            <View className="mb-2">
              <Text className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">
                API Base URL
              </Text>
              <Text className="text-xs font-mono text-indigo-300" numberOfLines={2}>
                {activeApiUrl}
              </Text>
            </View>
            <View>
        ...
        <View className="bg-gray-800 rounded-3xl p-5 border border-gray-700 mb-6 shadow-lg">
          ...
          >
            ...
            {mode === 'production' && <CheckCircle size={20} color="#6366f1" />}
          </TouchableOpacity>
          ...
          {/* Local Host Input */}
          {mode === 'local' && (
            <View className="mt-5 pt-4 border-t border-gray-700/80">
              <Text className="text-xs font-bold text-gray-300 mb-2">
              ...
              <TextInput
                ...
                className="bg-gray-900 text-white font-mono text-sm px-4 py-3.5 rounded-2xl border border-gray-700"
              />
              <View className="flex-row items-start gap-1.5 mt-2 px-1">
                <AlertCircle size={14} color="#9ca3af" className="mt-0.5" />
                <Text className="text-[11px] text-gray-400 flex-1 leading-tight">
        ...
        <TouchableOpacity
          ...
          disabled={saving}
          className="bg-indigo-600 active:bg-indigo-700 p-4.5 rounded-2xl flex-row items-center justify-center gap-2 shadow-lg shadow-indigo-600/30"
        >
          {saving ? (
