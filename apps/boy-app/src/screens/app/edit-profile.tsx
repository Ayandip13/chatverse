...
export default function EditProfileScreen() {
  ...
  const [avatar, setAvatar] = useState(
    getAvatarUrl(user?.avatar, user?.name, user?._id),
  );
  ...
  const handleSave = async () => {
    ...
    updateProfile(
      { name: name.trim(), bio: bio.trim(), avatar: finalAvatar },
      {
        onSuccess: () => {
          Alert.alert(
            "Profile Saved",
            "Your profile and picture have been updated!",
          );
          navigation.goBack();
        },
        onError: (err: any) => {
    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-gray-900" edges={["top"]}>
      ...
      >
        ...
        <View className="items-center mb-6">
          ...
          <TouchableOpacity
            onPress={pickImage}
            className="mt-2.5 flex-row items-center gap-1.5 bg-indigo-50 dark:bg-indigo-900/30 px-3.5 py-1.5 rounded-full border border-indigo-200 dark:border-indigo-800"
          >
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {(BOY_AVATAR_PRESETS || []).map((url, idx) => (
              <TouchableOpacity
                ...
                style={
                  avatar === url ? { transform: [{ scale: 1.05 }] } : undefined
                }
