      ...
      >
        ...
        <View className="bg-indigo-500 rounded-3xl p-6 shadow-lg shadow-indigo-500/30 mb-8">
          ...
          {isLoading ? (
            ...
            <View className="flex-row items-end gap-2 mb-2">
              ...
              <Text className="text-indigo-200 font-bold text-lg mb-2">
                Coins
              </Text>
            </View>
          )}
          <Text className="text-indigo-100 text-sm opacity-80">
            1 Coin = ₹1 (Approximate value)
          </Text>
        </View>
        ...
        <View className="flex-row gap-4 mb-8">
          <Button
            ...
            leftIcon={
              <PlusCircle color={theme.colors.background.light} size={20} />
            }
        ...
        <View className="bg-white dark:bg-gray-800 rounded-2xl p-2 shadow-sm border border-gray-100 dark:border-gray-700">
          {isTxLoading ? (
            ...
            transactionsData.map((tx: any, index: number) => {
              const isCredit = ["RECHARGE", "BONUS", "REFUND"].includes(
                tx.type,
              );
              return (
                <View
                ...
                >
                  <View className="flex-row items-center gap-4">
                    <View
                      className={`w-10 h-10 rounded-full items-center justify-center ${isCredit ? "bg-green-100 dark:bg-green-900/30" : "bg-red-100 dark:bg-red-900/30"}`}
                    >
                      {isCredit ? (
                        <ArrowDownLeft color={theme.colors.success} size={20} />
                      ) : (
                  ...
                  <Text
                    className={`font-bold text-base ${isCredit ? "text-green-500" : "text-gray-900 dark:text-white"}`}
                  >
                    {isCredit ? "+" : "-"}
                    {tx.amount}
                  </Text>
