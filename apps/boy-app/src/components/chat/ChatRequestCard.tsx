    ...
    <View className="bg-white dark:bg-gray-800 rounded-2xl p-4 mb-4 shadow-sm shadow-gray-200 dark:shadow-none border border-gray-100 dark:border-gray-700 flex-row items-center">
      ...
      <View className="flex-1">
        <View className="flex-row justify-between mb-1">
          ...
          <Text className="text-xs text-gray-400">{formatRelativeTime(request.createdAt)}</Text>
        </View>
      ...
      {request.status === 'PENDING' && (
        ...
        >
          <Text className="text-red-600 dark:text-red-400 text-xs font-bold">Cancel</Text>
        </TouchableOpacity>
      )}
    </View>
  );
