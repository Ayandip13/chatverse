...
interface MessageBubbleProps {
  message: Message & { status?: 'SENDING' | 'SENT' | 'DELIVERED' | 'READ' | 'FAILED' };
    ...
    <View className={`mb-3 w-full flex-row ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        ...
        {isReply && (
          <View className={`p-2.5 rounded-xl mb-2 border-l-4 ${
            isOwnMessage ? 'bg-indigo-700/60 border-indigo-300' : 'bg-gray-100 dark:bg-gray-700/60 border-indigo-500'
          }`}>
      ...
      <Modal visible={!!imageModalUrl} transparent animationType="fade">
        <View className="flex-1 bg-black/90 items-center justify-center p-4">
          ...
          {imageModalUrl && (
            <Image source={{ uri: imageModalUrl }} className="w-full h-4/5 rounded-2xl" resizeMode="contain" />
          )}
        </View>
      </Modal>
    </View>
