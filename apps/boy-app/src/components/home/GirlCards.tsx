...
import { Star, MessageCircle, ShieldCheck } from 'lucide-react-native';
...
export function GirlAvatarCard({ girl }: { girl: GirlProfile }) {
  ...
  return (
    <TouchableOpacity
      onPress={() =>
        navigation.navigate('GirlDetailScreen', { id: girl._id })
      }
    ...
    >
      <View className="relative w-16 h-16 p-0.5 rounded-full bg-gradient-to-tr from-indigo-500 via-rose-500 to-amber-400 items-center justify-center">
        <View className="w-full h-full rounded-full border-2 border-white dark:border-gray-900 overflow-hidden bg-gray-200 dark:bg-gray-800">
          <Image
            source={{
              uri: getAvatarUrl(girl.avatar, girl.name, girl._id),
            }}
      ...
      <Text
        className="mt-2 text-xs font-bold text-gray-800 dark:text-gray-200 max-w-[70px] text-center"
        numberOfLines={1}
      >
        {girl.name}
      </Text>
...
export function GirlDetailCard({ girl }: { girl: GirlProfile }) {
  ...
  return (
    <TouchableOpacity
      ...
      className="mr-4 w-[300px] h-32 rounded-2xl p-3 border border-gray-100 dark:border-gray-700/80 bg-white dark:bg-gray-800 flex-row items-center relative overflow-hidden"
      activeOpacity={0.88}
    >
      ...
      {/* LEFT: Rounded Square Profile Picture */}
      <View className="relative w-20 h-20 p-0.5 rounded-2xl bg-gradient-to-tr from-indigo-500 via-rose-500 to-amber-400 items-center justify-center">
        ...
        {girl.isOnline ? (
          <View className="absolute -bottom-1 -right-1 bg-emerald-500 w-4 h-4 rounded-full border-2 border-white dark:border-gray-800 items-center justify-center">
      ...
      <View className="flex-1 ml-3.5 flex-col justify-between h-full py-0.5">
        ...
        <View>
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center flex-1 mr-1">
              <Text
                className="font-extrabold text-gray-900 dark:text-white text-base mr-1"
                numberOfLines={1}
              >
                {girl.name}
              </Text>
          ...
          {/* Bio Snippet */}
          <Text
            className="text-xs font-medium text-gray-400 dark:text-gray-400 mt-1"
            numberOfLines={1}
          >
        ...
        <View className="flex-row items-center justify-between mt-1.5 pt-1.5 border-t border-gray-100 dark:border-gray-700/60">
          ...
          <View className="bg-indigo-600 px-3.5 py-1.5 rounded-xl flex-row items-center">
            <MessageCircle
              size={12}
              color="#ffffff"
              className="mr-1"
