...
import { useNotifications, useMarkRead, useMarkAllRead } from '../../src/hooks/useUser';
import { formatRelativeTime } from '../../src/utils/date';
import { EmptyState } from '../../src/components/ui/EmptyState';
import { Notification } from '../../src/api/userApi';
            ...
            >
              ...
              <View className="flex-1">
                <View className="flex-row justify-between mb-1">
                  <Text className={`font-bold text-base ${item.isRead ? 'text-gray-900 dark:text-white' : 'text-indigo-900 dark:text-indigo-100'}`}>
                    {item.title}
                  </Text>
                  {!item.isRead && <Circle size={10} fill="#4f46e5" color="#4f46e5" className="mt-1" />}
                </View>
                <Text className="text-gray-600 dark:text-gray-400 text-sm mb-2">{item.message}</Text>
                <Text className="text-xs text-gray-400">{formatRelativeTime(item.createdAt)}</Text>
              </View>
            </TouchableOpacity>
          )}
          onEndReached={() => {
