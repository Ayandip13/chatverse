...
import { ArrowLeft, History, XCircle, AlertCircle, CheckCircle2 } from 'lucide-react-native';
import { useRouter } from 'expo-router';
import { useMyWithdrawals, useCancelWithdrawal } from '../../../src/hooks/useWithdrawals';
    ...
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-900" edges={['top']}>
      ...
      <View className="flex-row items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-bold text-slate-900 dark:text-white">Withdrawal History</Text>
        </View>
      </View>
      ...
      <View className="bg-white dark:bg-slate-900 px-6 py-3 border-b border-slate-200 dark:border-slate-800">
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Text className={`text-xs font-bold ${selectedTab === item ? 'text-white' : 'text-slate-600 dark:text-slate-300'}`}>
                {item}
              </Text>
            </TouchableOpacity>
          )}
        ...
        ListEmptyComponent={
          isLoading ? (
            ...
            <View className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-slate-700 items-center justify-center">
              ...
              <Text className="text-base font-bold text-slate-800 dark:text-slate-200">No History Found</Text>
              <Text className="text-xs text-slate-400 text-center mt-1">
                You have no withdrawal records under the "{selectedTab}" status tab.
              </Text>
            </View>
          )
        }
          ...
          <View className="bg-white dark:bg-slate-800 p-5 rounded-3xl mb-4 border border-slate-200 dark:border-slate-700 shadow-sm">
            <View className="flex-row items-center justify-between mb-2">
              ...
              {item.status === 'PENDING' && (
                ...
                >
                  <Text className="text-rose-600 dark:text-rose-400 text-xs font-bold">Cancel</Text>
                </TouchableOpacity>
              )}
            </View>

            <Text className="text-xs text-slate-500 dark:text-slate-400 mb-2">
              Requested: {new Date(item.requestedAt || item.createdAt).toLocaleString()} • {item.paymentMethod}
            </Text>
            ...
            {item.paymentMethod === 'UPI' ? (
              ...
              <Text className="text-xs font-mono font-medium text-slate-700 dark:text-slate-300">
                Bank: {item.bankDetails.accountName} ({item.bankDetails.accountNumber.slice(-4)}) • {item.bankDetails.ifscCode}
              </Text>
            ) : null}
            ...
            {item.status === 'REJECTED' && item.rejectionReason && (
              <View className="mt-3 bg-red-50 dark:bg-red-900/20 p-3 rounded-xl border border-red-100 dark:border-red-900/50 flex-row items-start gap-2">
                ...
                <View className="flex-1">
                  <Text className="text-xs font-bold text-red-700 dark:text-red-400">Rejection Reason:</Text>
                  <Text className="text-xs text-red-600 dark:text-red-300">{item.rejectionReason}</Text>
                </View>
              </View>
            )}
