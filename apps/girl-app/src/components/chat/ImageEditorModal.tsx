  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      ...
      <View className="flex-1 bg-black">
          ...
          <View className="flex-row items-center gap-3">
            ...
            <TouchableOpacity
              ...
              className={`w-10 h-10 rounded-full items-center justify-center ${
                showEmojiPicker ? 'bg-pink-600' : 'bg-white/20'
              }`}
              activeOpacity={0.7}
            >
            ...
            <TouchableOpacity
              ...
              className={`w-10 h-10 rounded-full items-center justify-center ${
                isDrawing ? 'bg-pink-600' : 'bg-white/20'
              }`}
              activeOpacity={0.7}
            >
        ...
        {showEmojiPicker && (
          <View className="bg-slate-900/95 border-b border-slate-800 z-30 pb-2">
            {/* Category tabs */}
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-3 py-1.5 border-b border-slate-800/80">
              {EMOJI_CATEGORIES.map((cat) => (
                <TouchableOpacity
                  key={cat.id}
                  onPress={() => setStickerCategory(cat.id)}
                  className={`px-2.5 py-1 mr-1 rounded-full flex-row items-center gap-1 ${
                    stickerCategory === cat.id ? 'bg-pink-600' : 'bg-slate-800'
                  }`}
                >
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="px-3 pt-2">
              {currentCategoryObj.emojis.map((emoji, index) => (
                <TouchableOpacity
                  ...
                  className="p-2 mr-1 active:bg-slate-800 rounded-xl"
                >
        ...
        >
          ...
          >
            {/* Caption Input Pill */}
            <View className="flex-1 bg-slate-800/90 rounded-full px-4 py-2.5 flex-row items-center border border-slate-700">
              <TextInput
                value={caption}
            ...
            <TouchableOpacity
              ...
              className="w-13 h-13 rounded-full items-center justify-center shadow-lg bg-pink-600"
              activeOpacity={0.8}
            >
              {isUploading ? (
