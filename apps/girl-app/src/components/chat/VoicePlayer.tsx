    ...
    <View className="flex-row items-center gap-3 py-1 px-1 min-w-[200px]">
      <TouchableOpacity
        ...
        className={`w-10 h-10 rounded-full items-center justify-center ${
          isOwnMessage ? 'bg-white/20' : 'bg-rose-100 dark:bg-rose-950'
        }`}
      >
        {isBuffering ? (
          <ActivityIndicator size="small" color={isOwnMessage ? '#ffffff' : '#e11d48'} />
        ) : isPlaying ? (
          <Pause size={18} color={isOwnMessage ? '#ffffff' : '#e11d48'} />
        ) : (
          <Play size={18} color={isOwnMessage ? '#ffffff' : '#e11d48'} className="ml-0.5" />
        )}
      </TouchableOpacity>

      <View className="flex-1 justify-center">
        <View className={`h-1.5 w-full rounded-full overflow-hidden ${isOwnMessage ? 'bg-white/30' : 'bg-slate-200 dark:bg-slate-700'}`}>
          <View
            className={`h-full ${isOwnMessage ? 'bg-white' : 'bg-rose-500'}`}
            style={{ width: `${progressPercent}%` }}
          />
        </View>

        <View className="flex-row justify-between items-center mt-1">
          <Text className={`text-[10px] font-mono ${isOwnMessage ? 'text-rose-100' : 'text-slate-500 dark:text-slate-400'}`}>
            {isPlaying || currentTime > 0 ? formatSeconds(currentTime) : formatSeconds(totalDuration)}
          </Text>
          <View className="flex-row items-center gap-0.5">
            <Volume2 size={10} color={isOwnMessage ? '#ffe4e6' : '#94a3b8'} />
            <Text className={`text-[9px] font-bold ${isOwnMessage ? 'text-rose-200' : 'text-slate-400'}`}>Voice Note</Text>
          </View>
        </View>
      </View>
    </View>
