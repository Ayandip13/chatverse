  ...
  const pickImage = async () => {
    try {
      ...
      if (!permissionResult.granted) {
        Alert.alert(
          "Permission Denied",
          "Permission to access gallery is required to select a profile picture.",
        );
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (avatarRes.data?.data?.avatar) {
          useAuthStore
            .getState()
            .updateUser({ avatar: avatarRes.data.data.avatar });
        }
      } catch (uploadError) {
        ...
        <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 40 }}>
            ...
            >
              {avatarUri ? (
                ...
                <View className="items-center justify-center">
                  ...
                  <Text className="text-[10px] font-bold text-pink-500 mt-1">
                    UPLOAD PHOTO
                  </Text>
                </View>
              )}
            ...
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <Input
                  ...
                  leftIcon={
                    <FileText color={theme.colors.text.muted.light} size={20} />
                  }
                />
              )}
            />
                ...
                <View className="mb-6 mt-2">
                  ...
                  >
                    ...
                    <Text className="flex-1 text-slate-700 dark:text-slate-300 text-sm">
                      I agree to Creator Verification Terms and Platform
                      Guidelines
                    </Text>
                  </TouchableOpacity>
          ...
          <View className="p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl mb-6 flex-row items-start gap-3">
            ...
            <Text className="flex-1 text-xs text-indigo-900 dark:text-indigo-200 leading-relaxed font-medium">
              After submitting, your application will enter{" "}
              <Text className="font-bold">Pending Verification</Text>. An
              administrator will review your real profile picture and phone
              number before activating your profile.
            </Text>
          </View>
