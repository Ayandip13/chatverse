...
export function getAvatarUrl(
  ...
  if (
    ...
    if (cleanUrl.startsWith("/uploads") || cleanUrl.startsWith("uploads")) {
      const apiUrl =
        process.env.EXPO_PUBLIC_API_URL || "http://192.168.0.105:5000/api/v1";
  ...
  const index =
    Math.abs(hash) %
    (role === "BOY" ? DEFAULT_BOY_AVATARS.length : DEFAULT_GIRL_AVATARS.length);
  return role === "BOY"
    ? DEFAULT_BOY_AVATARS[index]
    : DEFAULT_GIRL_AVATARS[index];
}
