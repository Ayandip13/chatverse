...
export const getGirlsQuerySchema = z.object({
  query: z.object({
    ...
    online: z.enum(['true', 'false']).optional().transform(v => v === 'true'),
    favorites: z.enum(['true', 'false']).optional().transform(v => v === 'true'),
    rating: z.string().optional().transform(v => (v ? parseFloat(v) : undefined)),
    recentlyJoined: z.enum(['true', 'false']).optional().transform(v => v === 'true'),
    recommended: z.enum(['true', 'false']).optional().transform(v => v === 'true'),
