...
export const registerChatHandlers = (io: Server, socket: AuthenticatedSocket) => {
  ...
  socket.on(
    'chat:send_message',
    async (payload: { chatId: string; content: string; tempId?: string }, callback) => {
      try {
        ...
        chatSessionService.processMessageDeduction(chatId, userId, io).catch((err: Error) => {
        ...
        if (callback) callback({ error: err.message, tempId: payload.tempId });
      }
    },
  );
  ...
  socket.on('chat:typing_start', (payload: { chatId: string }) => {
    socket
      ...
      .emit('chat:typing_start', { chatId: payload.chatId, userId });
  });

  socket.on('chat:typing_stop', (payload: { chatId: string }) => {
    socket
      .to(`chat:${payload.chatId}`)
      .emit('chat:typing_stop', { chatId: payload.chatId, userId });
  });

  socket.on('chat:read', (payload: { chatId: string; messageId: string }) => {
    socket
      .to(`chat:${payload.chatId}`)
      .emit('chat:read_receipt', { chatId: payload.chatId, messageId: payload.messageId, userId });
  });
