...
export const Users = () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async (payload: { userId: string; status: string; reason: string }) => {
      const response = await apiClient.patch(`/admin/users/${payload.userId}/status`, {
  ...
  const handleConfirmAction = () => {
    ...
    if ((targetStatus === 'SUSPENDED' || targetStatus === 'BANNED') && !reason.trim()) {
      toast.error('Reason is required for this action');
      return;
    }
    updateStatusMutation.mutate({ userId: selectedUser._id, status: targetStatus, reason });
  };
    ...
    >
      ...
      <Card className="border-none shadow-card overflow-hidden">
        {isLoading && !users.length ? (
          ...
          <EmptyState
            title="No boys found"
            description={search || statusFilter ? 'Try adjusting your search or filter criteria.' : 'No registered boys available.'}
          />
        ) : (
          <div className="overflow-x-auto">
            <Table headers={['User Details', 'Contact', 'Joined Date', 'Status', 'Actions']}>
              {users.map((user: any) => (
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-textSecondary-light dark:text-textSecondary-dark">{user.role}</div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
        ...
        footer={
          <>
            ...
            <Button
              ...
              className={targetStatus === 'BANNED' ? 'bg-danger hover:bg-danger/90' : targetStatus === 'SUSPENDED' ? 'bg-warning hover:bg-warning/90' : ''}
            >
      ...
      >
        <div className="space-y-4 pt-4">
          <p className="text-textSecondary-light dark:text-textSecondary-dark">
            You are about to change <strong>{selectedUser?.name}</strong>'s status to{' '}
          ...
          <div>
            <label className="text-sm font-medium text-textMain-light dark:text-textMain-dark block mb-2">
              Reason / Admin Notes {targetStatus !== 'ACTIVE' && <span className="text-danger">*</span>}
            </label>
            <textarea
