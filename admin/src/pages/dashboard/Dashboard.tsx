...
export const Dashboard = () => {
  ...
  return (
    <PageLayout
      ...
      action={
        <Button onClick={handleRefresh} variant="secondary" className="gap-2">
          <RefreshCcw
            className={`w-4 h-4 ${isFetchingMetrics ? "animate-spin" : ""}`}
          />
          Refresh Data
        </Button>
      }
    >
      ...
      {/* Quick Actions */}
      <div className="mb-8">
        <h2 className="text-lg font-semibold text-textMain-light dark:text-textMain-dark mb-4">
          Quick Actions
        </h2>
        ...
        <Card className="border-none shadow-card">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => (window.location.href = "/girls")}
            >
              View All
            </Button>
          </CardHeader>
            ...
            {isLoadingGirls ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-12 bg-surface-dark/5 animate-pulse rounded-md"
                  />
                ))}
              </div>
            ) : recentGirls?.length ? (
              <Table headers={["User", "Date", "Status"]}>
                {recentGirls.map((user: any) => (
                  <TableRow key={user._id}>
                    <TableCell>
                      <div className="font-medium text-textMain-light dark:text-textMain-dark">
                        {user.name}
                      </div>
