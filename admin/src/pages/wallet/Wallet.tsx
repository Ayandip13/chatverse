...
import { Wallet as WalletIcon, RefreshCcw, AlertCircle, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
...
const getTransactionBadgeVariant = (type: string): BadgeVariant => {
  switch (type) {
    case 'RECHARGE':
    case 'GIRL_EARNING': return 'success';
    case 'CHAT_DEBIT':
    ...
    >
      ...
      <Card className="border-none shadow-card overflow-hidden">
        {isLoading && !transactions.length ? (
          ...
          <EmptyState
            icon={<WalletIcon className="w-10 h-10 text-textMuted-light dark:text-textMuted-dark" />}
            title="No transactions found"
            description={typeFilter ? 'No transactions match the selected type.' : 'No financial transactions logged yet.'}
          />
        ) : (
          <div className="overflow-x-auto">
            <Table headers={['User', 'Type', 'Amount', 'Description', 'Timestamp']}>
