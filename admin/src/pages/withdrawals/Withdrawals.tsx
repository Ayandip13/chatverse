  ...
  const updateWithdrawalMutation = useMutation({
    mutationFn: async (payload: {
      ...
      const response = await apiClient.patch(
        `/admin/withdrawals/${payload.id}/status`,
        payload,
      );
      return response.data;
    },
  ...
  const handleConfirmAction = () => {
    ...
    updateWithdrawalMutation.mutate({
      ...
      transactionReference:
        actionType === "MARK_PAID" ? transactionReference.trim() : undefined,
    });
  };
    ...
    >
      ...
      <Card className="border-none shadow-card overflow-hidden">
        {isLoading && !requests.length ? (
          ...
          <EmptyState
            icon={
              <CreditCard className="w-10 h-10 text-textMuted-light dark:text-textMuted-dark" />
            }
            title="No payout requests found"
            description={
              statusFilter
                ? "No payout requests match the selected status."
                ...
                <TableRow key={req._id}>
                    ...
                    <div className="text-sm">
                      ...
                      req.bankDetails ? (
                        <div className="text-xs font-mono mt-1 text-textMain-light dark:text-textMain-dark">
                          <div>
                            <strong>Acc:</strong> {req.bankDetails.accountName}{" "}
                            ({req.bankDetails.accountNumber})
                          </div>
                          <div>
                            <strong>IFSC:</strong> {req.bankDetails.ifscCode}
                          </div>
                        </div>
                      ) : (
                  ...
                  <TableCell>
                    <div>
                      ...
                      <div className="text-[10px] text-textSecondary-light">
                        Net Payout: ₹{req.netAmount || req.amount}
                      </div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-sm text-textSecondary-light dark:text-textSecondary-dark">
                    {format(
                      new Date(req.requestedAt || req.createdAt),
                      "MMM d, yyyy HH:mm",
                    )}
                  </TableCell>
                    ...
                    {req.status === "PENDING" ? (
                      ...
                      <span className="text-xs text-textMuted-light dark:text-textMuted-dark italic">
                        {req.status === "COMPLETED"
                          ? `Ref: ${req.transactionReference || "Completed"}`
                          : `Reason: ${req.rejectionReason || "N/A"}`}
                      </span>
                    )}
                  </TableCell>
                </TableRow>
      ...
      >
        <div className="space-y-4 pt-2">
          <p className="text-sm text-textSecondary-light dark:text-textSecondary-dark">
            You are about to execute action <strong>{actionType}</strong> on
            withdrawal request of{" "}
            <strong className="text-emerald-600 dark:text-emerald-400 font-mono">
              ₹{selectedReq?.amount}
            </strong>{" "}
            for <strong>{selectedReq?.userId?.name || "Creator"}</strong>.
          </p>
          ...
          <div className="p-3 bg-surface-light dark:bg-surface-dark rounded-xl border border-border-light dark:border-border-dark text-xs space-y-1">
            <p>
              ...
              {selectedReq?.paymentMethod || "UPI"}
            </p>
            {selectedReq?.paymentMethod === "BANK_TRANSFER" &&
            selectedReq?.bankDetails ? (
              <>
                <p>
                ...
                <p>
                  ...
                  <span className="font-mono">
                    {selectedReq.bankDetails.ifscCode}
                  </span>
                </p>
              </>
            ) : (
            ...
            {actionType === "REJECT" && (
              <p className="text-rose-500 font-semibold pt-1">
                * Rejecting this request will automatically refund ₹
                {selectedReq?.amount} back to the creator's available wallet
                balance.
              </p>
            )}
          </div>
