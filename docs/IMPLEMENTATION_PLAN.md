...
- **Payment Failures / Webhook Delays:** 
  - *Mitigation:* Idempotency keys on transactions; polling fallback on client side if webhook is delayed.
- **Socket Disconnections / State Loss:** 
  - *Mitigation:* Persist chat states to MongoDB; clients fetch missed messages via REST on reconnect.
- **Race Conditions in Wallet Deductions:** 
  - *Mitigation:* Strict atomic `$inc` updates in MongoDB; never read-calculate-save.
- **App Store Rejections (NSFW/Digital Goods):** 
  - *Mitigation:* Strict moderation policies documented; ensure Razorpay is permitted or use IAP (In-App Purchases) if mandated by Apple/Google.
